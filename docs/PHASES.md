...
| **2** | Plans, entitlements, scope tiering, per-site grants. WPistic ecosystem-key adapter. Remove `$tier` from `KeyStore::create`. | A Free-tier key cannot call `db:write`. A site grant can narrow below the plan. | Catalogue, tiering and adapter landed and tested; API handlers outstanding. |
...
- `apps/api` — the `UsageCounter` Durable Object with reserve/settle, the
  entitlement adapter, and the full route surface declared.
- `apps/scheduler` — the per-job `JobScheduler` Durable Object with alarm, lock,
  overlap and catch-up.

Declared but not implemented, and honest about it: API handlers return 501 with
the phase that will implement them; the scheduler's queue consumer throws with a
pointer to `docs/SCHEDULER.md`; `apps/mcp` serves `/health` and nothing else
until `cloud/src` moves in.
